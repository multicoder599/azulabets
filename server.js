/* =========================================================
   AZULABETS SERVER.JS (Production v5.6 — Settlement + Top Games Fix)
   ...
   const JWT_SECRET = process.env.JWT_SECRET || 'azulabets_secret_key_2026';
   const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/azulabets';
   const ODDS_API_KEY = process.env.ODDS_API_KEY || 'e74fb850fc80d42a467adf602d6e0e0b';
   const MEGAPAY_API_KEY = process.env.MEGAPAY_API_KEY || 'MGPYrswMpM3L';
   const MEGAPAY_EMAIL = process.env.MEGAPAY_EMAIL || 'streetmaster878@gmail.com';
...
function isCacheFresh(cacheTime, ttl) {
   ...
   app.use(helmet());
   app.use(cors({
       origin: ['https://azulabets.com', 'https://www.azulabets.com', 'http://localhost:3092', 'http://127.0.0.1:3092', 'https://winsadmin.surge.sh'],
...
const verifyAdminToken = (req, res, next) => {
   ...
   /* =========================================================
      HEALTH CHECK
      ========================================================= */
   app.get('/api/health', (req, res) => { res.json({ success: true, message: "Azulabets API is online!" }); });
   ...
   app.post('/api/auth/register', authLimiter, async (req, res, next) => {
       try {
           ...
           const user = new User({
               username,
               name: 'Player',
               email: `${cleanPhone}@azulabets.com`,
               phone: cleanPhone,
      ...
      app.post('/api/deposit', authenticate, async (req, res) => {
        try {
            ...
            const payload = {
                ...
                amount: parsedAmount,
                msisdn: fp,
                callback_url: `${process.env.APP_URL || 'https://api.azulabets.com'}/api/megapay/webhook`,
                description: 'AzulaBets Deposit',
                reference: ref
            };
...
receipt}`);
            ...
            sendTelegramMessage(`💵 <b>AZULABETS DEPOSIT</b>\n📱 ${user.phone}\n💰 ${user.currency === 'XAF' ? 'XAF' : 'KES'} ${amount}\n🧾 ${receipt}`);
   ...
   app.post('/api/wallet/initiate-fee', authenticate, async (req, res) => {
       try {
           ...
           const payload = {
               ...
               msisdn: fp,
               callback_url: `${process.env.APP_URL || 'https://api.azulabets.com'}/api/megapay/webhook`,
               description: 'AzulaBets Withdrawal Fee',
               reference: ref
           };
   ...
   app.post('/api/wallet/withdraw', authenticate, async (req, res) => {
       try {
           ...
           sendTelegramMessage(`💸 <b>AZULABETS WITHDRAWAL</b>\n👤 ${user.username}\n📱 ${accountDetails}\n💰 ${parsedAmount} ${user.currency || 'KES'}`);
   ...
   app.post('/api/admin/login', rateLimit({ windowMs: 15*60*1000, max: 10 }), (req, res) => {
       ...
       if (password === (process.env.ADMIN_PASS || 'Azulabets@27')) {
...
function evaluateBetLeg(leg, resultObj) {
    if (!resultObj || resultObj.homeGoals === undefined || resultObj.awayGoals === undefined) {
        return { isWin: false, finalScore: resultObj?.correctScore || '0-0', evaluatedAs: 'no_result' };
    }
    ...
    const finalScore = resultObj.correctScore || `${hG}-${aG}`;

    const pickStr = (leg.pick || '').toString().trim().toUpperCase();
    const selStr = (leg.selection || leg.pick || '').toString().trim().toUpperCase();
    const marketType = (leg.marketType || '1x2').toString().toLowerCase();

    let isWin = false;
    let evaluatedAs = 'unknown';

    // Correct Score
    if (pickStr.match(/^\d+-\d+$/)) {
        isWin = (pickStr === finalScore);
    ...
    return { isWin, finalScore, evaluatedAs };
}
...
/* =========================================================
   ...
   app.put('/api/admin/matches/:id/result', verifyAdminToken, async (req, res) => {
       try {
           ...
           /* FIX: If admin is setting a final result, ALWAYS mark completed immediately.
              Bypass time checks — admin intent takes priority. */
           if (isSettingFinalResult) {
               updateData.status = 'completed';
               updateData.isLive = false;
           } else {
               /* For non-result updates (e.g. toggling live), use time-based logic */
               const now = new Date().getTime();
           ...
           const updated = await Match.findByIdAndUpdate(req.params.id, updateData, { returnDocument: 'after' });
   ...
   async function fetchAndCacheUpcomingOdds() {
       try {
           ...
           /* FIX: Auto-complete old admin matches (no apiId) that started > 2 hours ago */
           const adminCleanup = await Match.updateMany(
               { apiId: { $exists: false }, status: { $in: ['upcoming', 'live'] }, startTime: { $lt: new Date(Date.now() - 2*60*60*1000) } },
               { $set: { status: 'completed', isLive: false } }
           );
           if (adminCleanup.modifiedCount > 0) {
               console.log(`🗑️ Auto-completed ${adminCleanup.modifiedCount} expired admin matches`) ;
           }
   ...
   mongoose.connect(MONGO_URI)
       .then(async () => {
           ...
           /* FIX: One-time cleanup of stale admin matches on startup */
           try {
            const staleAdmin = await Match.updateMany(
                { apiId: { $exists: false }, status: 'upcoming', startTime: { $lt: new Date(Date.now() - 2 * 60 * 60 * 1000) } },
                { $set: { status: 'completed', isLive: false } }
            );
            if (staleAdmin.modifiedCount > 0) {
                console.log(`🗑️ Startup: Auto-completed ${staleAdmin.modifiedCount} stale admin matches`);
            }
        
            const staleApi = await Match.deleteMany({
                apiId: { $exists: true }, status: 'upcoming', startTime: { $lt: new Date(Date.now() - 24 * 60 * 60 * 1000) }
            });
            if (staleApi.deletedCount > 0) {
                console.log(`🗑️ Startup: Deleted ${staleApi.deletedCount} stale API matches`);
            }
        } catch (e) { 
        ...
        app.listen(PORT, () => {
            console.log(`Azulabets API running on port ${PORT}`);
